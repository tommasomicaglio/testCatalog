...
//        return "";
        ...
        for (int i = 0; i < sa.length; i++) {
            ...
            String[] sd = sa[i].split("</name>");

            if (sd[0].equals(article)) {
                String[] sr = sa[i].split("<prx>");
                String[] sf = sr[1].split("</prx>");
                String price = sf[0];
                sa = sa[i].split("<qty>");
                String[] sg = sa[1].split("</qty>");
                String qty = sg[0];
                return article+" > prx: $"+price + " qty: " + qty;
